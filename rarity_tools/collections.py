...
class CollectionsManager:
    ...
    def __init__(self):
        tz = timezone('EST')
        dt = str(datetime.datetime.now(tz).date()).replace("-", "")
        self.filename = f"rarity-{dt}.json"
    ...
    def cache(self, dir_path="cache"):
        with open(f"{dir_path}/{self.filename}", 'w') as f:
            json.dump(self.collections, f)

    def persist_s3(self, s3_bucket="nft-data-tracker", s3_prefix="rarity"):
        client = boto3.client('s3')
        client.put_object(Body=json.dumps(self.collections), Bucket=s3_bucket, Key=f'{s3_prefix}/{self.filename}')
